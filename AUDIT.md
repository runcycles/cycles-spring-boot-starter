...
v0.3.1. Heartbeat redesigned around a clock-skew-free lead estimate: `extend_by_ms` is relative to the current `expires_at_ms` per spec, so the old extend-every-`ttl/2`-tick loop drifted expiry `+ttl/2` per beat (zombie budget lockup on process death) and burned the server's capped `extension_count` twice as fast. Each tick now computes `leadMs = (knownExpiry − initialExpiry) + ttl − elapsedMonotonicMs` and extends by `ttl` only when the lead falls below `1.5·ttl` (catch-up ticks self-skip). Failed extends replay the same idempotency key next tick; 410/`RESERVATION_EXPIRED`/`RESERVATION_FINALIZED`/`MAX_EXTENSIONS_EXCEEDED`/`TENANT_CLOSED`/`NOT_FOUND` cancel the heartbeat permanently; the 1000ms interval floor is removed (spec ttl min 1000 → interval ≥500ms). Tenant-capped TTLs are honored: the client captures the HTTP `Date` header on 2xx (`CyclesResponse.getDateMs()`) and the heartbeat runs off `effectiveTtl = clamp(expires_at_ms − dateMs, 1000, requestedTtl)` — both server-frame, skew-free — for its interval, lead math, and `extend_by_ms`. Separately, when the estimate is committed as actual (`useEstimateIfActualNotProvided` fallback), the commit metadata now carries `actual_source=estimate` (also flows into the `/v1/events` recovery body).
