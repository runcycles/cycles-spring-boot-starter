...
v0.3.1. Heartbeat redesigned around a conservative lead lower bound (v2.2): `extend_by_ms` is relative to the current `expires_at_ms` per spec, so the old extend-every-`ttl/2`-tick loop drifted expiry `+ttl/2` per beat (zombie budget lockup on process death) and burned the server's capped `extension_count` twice as fast. Each beat now computes `leadMinMs = grantsSum − elapsedMonotonicMs` (grants = differences of successive returned `expires_at_ms`, same server frame — no cross-clock math; starts at 0), skips only at `leadMin ≥ 1.5·lastGrant`, and otherwise extends by the *requested* ttl; beats are one-shot and self-rescheduling (next delay `clamp(lastGrant/2, 500, requestedTtl/2)`), which removes the fixed-rate catch-up hazard by construction. The HTTP `Date` header (`CyclesResponse.getDateMs()`) is demoted to a first-beat cadence hint only — `firstDelay = min(requestedTtl/2, 30s, (expires_at_ms − dateMs)/2 when positive)`, no upward clamp — because RFC 9110 `Date` is a whole-second best-effort timestamp intermediaries may replace and shares no guaranteed clock with `expires_at_ms` (Redis `TIME` vs servlet container in cycles-server). Failed extends replay the same idempotency key next beat; 410/`RESERVATION_EXPIRED`/`RESERVATION_FINALIZED`/`MAX_EXTENSIONS_EXCEEDED`/`TENANT_CLOSED`/`NOT_FOUND` cancel the heartbeat permanently; the 1000ms interval floor is removed (spec ttl min 1000 → steady-state delay ≥500ms). Separately, when the estimate is committed as actual (`useEstimateIfActualNotProvided` fallback), the commit metadata now carries `actual_source=estimate` (also flows into the `/v1/events` recovery body). 542 tests pass; JaCoCo ≥95% INSTRUCTION gate met (98.35%).
